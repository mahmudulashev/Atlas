...
final class Explainer: ObservableObject {
    ...
    /// The explanation derived from the graph alone.
    ///
    /// Lives in the engine now — it is wording, and the Windows client needs
    /// the same sentences. Kept here as a forward so the views that ask for it
    /// do not have to know that.
    static func staticExplanation(node: GraphNode, graph: CodeGraph, t: L10n) -> String {
        Explanation.fromGraph(node: node, graph: graph, t: t)
    }
